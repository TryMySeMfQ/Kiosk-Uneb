...
function playIntroVideo() {
    ...
    quadroAvisos.innerHTML = `
        <video id="introVideo" autoplay muted playsinline style="width:100%; height:100%; opacity: 0; transition: opacity 1s;">
    ...
    //habilita o som apos interargir com o video
...
function displayNews() {
    ...
    function nextNews() {
      if (currentIndex < noticias.length) {
        ...
        img.src = noticias[currentIndex];
        img.alt = `Notícia ${currentIndex + 1}`;
        ...
        img.onload = () => {
          ...
          currentIndex++;
          setTimeout(nextNews, 20000); // mostra cada imagem por 20 segundos
        };
  
        // Se der erro, pula para a próxima imagem
        img.onerror = () => {
          console.warn(`❌ Falha ao carregar imagem: ${img.src}`);
          currentIndex++;
          nextNews(); // tenta a próxima imediatamente
        };
      } else {
        // Acabaram as notícias → mostra os horários
        quadroAvisos.style.display = "none";
        const tableContainer = document.getElementById("horarios");
        tableContainer.style.display = "block";
        updateSchedule(); // atualiza os dados da tabela
      }
    }
    ...
    function filterCSVData(data, currentTime) {
        ...
        if (currentTime.getTime() >= morningStart.getTime() && currentTime.getTime() <= morningEnd.getTime()) {
            filteredData = data.filter(row => {
                const startTime = csvTimeToDate(row.inicio);
                return startTime && startTime.getTime() >= morningStart.getTime() && startTime.getTime() <= morningEnd.getTime();
            });
        } else if (currentTime.getTime() > afternoonStart.getTime() && currentTime.getTime() <= afternoonEnd.getTime()) {
            filteredData = data.filter(row => {
                const startTime = csvTimeToDate(row.inicio);
                return startTime && startTime.getTime() >= afternoonStart.getTime() && startTime.getTime() <= afternoonEnd.getTime();
            });
        } else if (currentTime.getTime() > nightStart.getTime() && currentTime.getTime() <= nightEnd.getTime()) {
            filteredData = data.filter(row => {
                const startTime = csvTimeToDate(row.inicio);
                return startTime && startTime.getTime() >= nightStart.getTime() && startTime.getTime() <= nightEnd.getTime();
            });
        }
    ...
    function updateSchedule() {
        ...
        if (diaDaSemana === 'domingo' || (diaDaSemana === 'sabado' && now.getHours() >= 14)) {
            showDayOff();
            return;
        }
        ...
        if (now.getTime() < startDayTime.getTime() || now.getTime() > endDayTime.getTime()) {
            showDayOff();
            return;
        }
            ...
            Papa.parse(csvFile, {
                ...
                complete: function (results) {
                    ...
                    if (currentClasses.length > 0) {
                        ...
                        tableBody.innerHTML = '<tr><td colspan="6">Não há aulas no momento.</td></tr>';
                    }
...
function startSequence() {
    ...
    document.getElementById('quadroAvisos').style.display = 'none';
    //
    ...
    }, 15 * 30 * 1000); // 15 minutos em milissegundos
}
...
// Mostra o quadro de horários e oculta os avisos no carregamento
//document.getElementById('horarios').style.display = 'block';
//document.getElementById('quadroAvisos').style.display = 'none';


// Atualiza a tabela de horários regularmente
setupScheduleUpdates();

// Aguarda 15 minutos e inicia sequência de vídeo e avisos
console.log("DOM carregado. A sequência de vídeo e avisos iniciará em 15 minutos.");
/* Mostra o quadro de horários e oculta os avisos no carregamento
const horarios = document.getElementById('horarios');
const quadroAvisos = document.getElementById('quadroAvisos');

if (horarios && quadroAvisos) {
